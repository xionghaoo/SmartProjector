...
final class DecodeHandler extends Handler {
    ...
    DecodeHandler(QRCodeView activity) {
    ...
    public void handleMessage(Message message) {
        switch (message.what) {
            case QRcodeMessage.decode:
                decode((byte[]) message.obj, message.arg1, message.arg2);
                break;
            case QRcodeMessage.quit:
    ...
    private void decode(byte[] data, int width, int height) {
        ...
        if (rawResult != null) {
            Message message = Message.obtain(mActivity.getCaptureHandler(), QRcodeMessage.decode_succeeded, rawResult);
            message.sendToTarget();
        } else {
            Message message = Message.obtain(mActivity.getCaptureHandler(), QRcodeMessage.decode_failed);
            message.sendToTarget();
        }
    }
}
