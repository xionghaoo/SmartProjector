 ...
 * @Date 2018-11-29
 **/
public class SwitchTouchView extends ControllerWidgetView<SwitchTouchConfig> implements Checkable, View.OnClickListener {

	private TextView name;
	private ImageView icon;
	private boolean isChecked = false;

	public SwitchTouchView(Context context) {
		this(context, null);
	}

	public SwitchTouchView(Context context, @Nullable AttributeSet attrs) {
		super(context, attrs);
		initSwitchTouchView();
	}

	private void initSwitchTouchView() {
		View view = inflateView(R.layout.item_project_controller_switch_touch);
	...
	public void setBlockId(String blockId) {
		SwitchTouchConfig config = getWidgetConfig();
	...
	@Override
	protected SwitchTouchConfig newConfig() {
		return new SwitchTouchConfig();
	}
	
	@Override
	protected WidgetType getType() {
		return WidgetType.WIDGET_UKTOUCHSWITCH;
	}
	
	@Override
	protected void applyConfig(SwitchTouchConfig config) {
		if(config == null) {
			name.setText(null);
			return;
		}
