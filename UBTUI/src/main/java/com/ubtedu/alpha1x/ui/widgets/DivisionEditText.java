...
public class DivisionEditText extends androidx.appcompat.widget.AppCompatEditText {
    ...
    public DivisionEditText(Context context, AttributeSet attrs) {
        ...
        if (typedArray != null) {
            mTotalLength = typedArray.getInteger(
                    R.styleable.DivisionEditText_totalLength, DEFAULT_BANK_CARD_LENGTH);
            mSperator = typedArray.getString(R.styleable.DivisionEditText_sperator);
            if (TextUtils.isEmpty(mSperator)) {
                mSperator = DEFAULT_DIVIDE_SYMBOL;
            }
            mType = typedArray.getInt(R.styleable.DivisionEditText_type, BANK_TYPE);
            if (mType == BANK_TYPE) {
                // 计算需要几个分隔符及每个对应的长度
