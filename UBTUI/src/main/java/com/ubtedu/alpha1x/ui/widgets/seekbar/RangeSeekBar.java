package com.ubtedu.alpha1x.ui.widgets.seekbar;
...
import static com.ubtedu.alpha1x.ui.widgets.seekbar.SeekBar.INDICATOR_ALWAYS_HIDE;
import static com.ubtedu.alpha1x.ui.widgets.seekbar.SeekBar.INDICATOR_ALWAYS_SHOW;
...
public class RangeSeekBar extends View {
    ...
    private int indicatorRightDrawableId;
    private int indicatorLeftDrawableId;
    ...
    private void initSeekBar(AttributeSet attrs) {
        ...
        if (thumbLeftDrawableId!=0){
            leftSB.setThumbDrawableId(thumbLeftDrawableId);
        }
        if (indicatorLeftDrawableId!=0){
            leftSB.setIndicatorDrawableId(indicatorLeftDrawableId);
        }
        if (seekBarMode != SEEKBAR_MODE_SINGLE) {
            if (thumbRightDrawableId != 0) {
                rightSB.setThumbDrawableId(thumbRightDrawableId);
            }
            if (indicatorRightDrawableId!=0){
                rightSB.setIndicatorDrawableId(indicatorRightDrawableId);
            }
        }
    }
    ...
    private void initAttrs(AttributeSet attrs) {
        try {
            ...
            thumbRightDrawableId=t.getResourceId(R.styleable.RangeSeekBar_rsb_thumb_right_drawable,0);
            thumbLeftDrawableId=t.getResourceId(R.styleable.RangeSeekBar_rsb_thumb_left_drawable,0);
            indicatorRightDrawableId=t.getResourceId(R.styleable.RangeSeekBar_rsb_indicator_right_drawable,0);
            indicatorLeftDrawableId=t.getResourceId(R.styleable.RangeSeekBar_rsb_indicator_left_drawable,0);
            t.recycle();
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//                        currTouchSB.slide(performClick);
    ...
    public void setIndicatorTextDecimalFormat(DecimalFormat format) {
        leftSB.setIndicatorTextDecimalFormat(format);
        if (seekBarMode == SEEKBAR_MODE_RANGE) {
            rightSB.setIndicatorTextDecimalFormat(format);
        }
    }
