...
public class CenterSnapHelper extends RecyclerView.OnFlingListener {
    ...
    private final RecyclerView.OnScrollListener mScrollListener =
            new RecyclerView.OnScrollListener() {
                ...
                public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                    Log.d("CenterSnapHelper", "dx:" + dx + " dy:" + dy);
                    if (dx != 0 || dy != 0) {
                        mScrolled = true;
                    }
                }
    ...
    void setupCallbacks() throws IllegalStateException {
        ...
        mRecyclerView.setOnGenericMotionListener(new View.OnGenericMotionListener() {
            @Override
            public boolean onGenericMotion(View v, MotionEvent event) {
//                if ((event.getSource() & InputDevice.SOURCE_CLASS_POINTER) != 0) {
//                    if (event.getAction() == MotionEvent.ACTION_SCROLL &&
//                            mRecyclerView.getScrollState() == SCROLL_STATE_IDLE) {//鼠标滑
//                        // 轮事件执行&&RecyclerView不是真正滑动
//                        mScrollListener.onScrollStateChanged(mRecyclerView, mRecyclerView.getScrollState());
//                    }
//                }
                return true;
            }
